...
### DOC-0054 — Permit authenticated Agent Credential self-restriction

- Date: 2026-08-12
- Mode: Refresh
- Status: Approved
- Affects: Canonical Agent Credential definition, Agent API boundary, credential security, generated OpenAPI guidance, ADR 0009, and the Agent Integration developer guide
- Evidence: User direction on 2026-08-12 to implement an authenticated self-restriction endpoint, instruct the agent to report the outcome, defer UI changes, and add no notification feature; endpoint, authentication, monotonic-expiry, revocation, preview-invalidation, rate-limit, and generated-contract PHPUnit coverage
- Decision: Add one narrow exception to the v1 prohibition on credential management. A live Agent Credential may submit a closed command that either moves only its own expiry earlier or revokes itself. It cannot target another credential, accept a Family identifier, extend or restore authority, change abilities or name, rotate a secret, or create credentials. Self-revocation retains non-secret metadata, records no revoking User, and invalidates unapplied previews. Publish workflow guidance in the generated OpenAPI operation requiring the agent to report the returned status and exact timestamps to the User; implement no application notification or UI in this increment. Keep documentation version `0.4.0` and defer publication verification.
- Rationale: Users may expose bearer secrets in plaintext when configuring an agent. A monotonic current-credential command reduces that exposure without granting any new authority or widening Family scope. Separating rate limits by action prevents a shortening loop from blocking final revocation, while OpenAPI guidance makes the intended agent workflow discoverable without pretending the application can guarantee agent compliance.
- Follow-up or review date: Revisit UI feedback separately as requested. Rebuild and review the configured documentation artifacts only when documentation publication resumes; this decision does not close Slice 0 production evidence.

### DOC-0055 — Permit immediate Agent entity-image upload

- Date: 2026-08-12
- Mode: Refresh
- Status: Approved
- Affects: Agent API v1 resource boundary, Cookbook media seam, credential abilities, Family isolation, rate limits, generated OpenAPI contract, ADRs 0010 and 0011, and the Agent Integration developer guide
- Evidence: User direction on 2026-08-12 to add one universal media-upload endpoint that applies exactly the same rules and workflow as the UI for entities that currently support at most one image; existing shared `ManageEntityMedia` and private normalized media storage implementation; authenticated multipart endpoint, ability, Family-isolation, replacement, archive, validation, rate-limit, and generated-contract PHPUnit coverage
- Decision: Add `POST /api/v1/media/{resource_type}/{id}` for `stores`, `ingredients`, and `recipes`, exactly matching the resource types whose web UI accepts an image upload. Store Sections use the predefined icon catalogue through their ordinary structured mutation contract and are intentionally excluded. Require `cookbook:write`, derive Family scope only from the live Agent Credential, accept exactly one multipart `image`, recheck live credential authority and issuer membership inside the transaction, and delegate to the same Cookbook media action and storage implementation as the web interface. Preserve the existing JPEG/PNG 5 MiB limit, safe decode, normalized private WebP variants, entity lock, single-image replacement, archive rules, and storage rollback. Supersede DOC-0052's no-source-dimension-limit policy for both UI and Agent callers: before GD allocation, reject dimensions above configured defaults of 8192 pixels per side or 25,000,000 total pixels. Treat this binary operation as an immediate narrow exception to Change Set-only mutation; do not add remote fetching, extraction, image reads or deletion, galleries, generic attachments, or media for unsupported entities. Apply a dedicated per-credential limit of 10 uploads per minute and publish an explicit closed multipart schema through Scramble.
- Rationale: Reusing the existing deep Cookbook media module gives web and Agent callers one validation, tenancy, normalization, replacement, and cleanup implementation. A direct multipart adapter is substantially smaller and safer than encoding binary data in immutable JSON Change Sets or persisting temporary untrusted files for preview, while `cookbook:write`, fixed credential scope, and a dedicated rate limit bound the new authority.
- Follow-up or review date: Consider authenticated media reads or deletion only through separately approved contracts. Keep documentation version `0.4.0`, defer publication verification, and keep Slice 0 live production evidence incomplete.
