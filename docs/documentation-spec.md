...
Family persistence, operator User provisioning, Family collaboration, reusable Current Family scoping, Store creation/listing/renaming/deletion, and reusable Store Section creation/listing are implemented. Store logos, optional Store Section icons, Store–Section association and ordering, Store Section deletion, and the remaining Recipe, Ingredient, meal-calendar, nutrition, and Shopping List domain described in `CONTEXT.md` are not implemented in the current application.
