...
The product boundary is this family-scoped cookbook and shopping-planning web application. A User may participate in multiple Families and works within one Current Family at a time. Each Family exclusively owns its Cookbook, Ingredients, Stores, Store Sections, Store Placements, Calendar Entries, and Saved Shopping Lists. The currently implemented boundary contains authentication, password recovery, passkey management, profile and security settings, appearance settings, a placeholder authenticated dashboard, operator-only User provisioning, complete Family Access collaboration workflows, reusable Current Family record scoping, Store create/list/rename/delete, reusable Store Section create/list/delete, per-Store Section association/removal/ordering with optimistic concurrency, and concrete packaged Ingredient creation/listing with canonical package quantities and derived display units. Store logos, Store Section icons, Ingredient editing, description, non-canonical input-unit selection and conversion, placement, media, nutrition, alternatives and archival, and the remaining Recipe, meal-calendar, Shopping List, and shopping-generation workflows remain intended functionality that is not yet implemented.
...
Family persistence, operator User provisioning, Family collaboration, reusable Current Family scoping, Store creation/listing/renaming/deletion, reusable Store Section creation/listing/deletion, Store–Section association/removal/ordering, and concrete Ingredient creation/listing with core package quantities are implemented. Store logos, optional Store Section icons, Ingredient editing, description, non-canonical input-unit selection and conversion, Store Placement, media, nutrition, alternatives and archival, and the remaining Recipe, meal-calendar, Shopping List, and Shopping Generation domain described in `CONTEXT.md` are not implemented in the current application.
