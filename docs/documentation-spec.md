...
Family persistence, operator User provisioning, Family collaboration, reusable Current Family scoping, Store creation/listing/renaming/deletion, reusable Store Section creation/listing/deletion, and Store–Section association/removal/ordering are implemented. Store logos, optional Store Section icons, Ingredient placement cleanup during Section deletion, and the remaining Recipe, Ingredient, meal-calendar, nutrition, and Shopping List domain described in `CONTEXT.md` are not implemented in the current application.
