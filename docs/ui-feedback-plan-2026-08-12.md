...
- [ ] **Finding 15 — layered relation creation (started).** Ingredient Store and Store Section selectors now offer async-loaded full create forms in layered dialogs. The unfinished Ingredient form remains intact; a created Store is selected immediately, and a created Section is atomically attached to the selected Store and then selected. Layered creation for Calendar Recipes and the remaining relation types is still pending.
- [ ] **Finding 16 — lazy relation search (initial scope delivered).** The reusable Current-Family search contract now covers Recipes, Stores, and Store Sections with debounced `q`, cursor continuation, a configurable 20-result default, request cancellation, and Store-constrained Sections. Calendar Recipe add/edit and Ingredient Store/Section selectors no longer receive eager catalogue payloads. Remaining eager relation selectors, including Simple Plan Recipes, Recipe Ingredients/Tags, Ingredient Alternatives, and Store Section association, still need migration before the all-relations finding is complete.
...
| Calendar refinements             | Calendar Vitest; ESLint, Prettier, TypeScript, and production build                                                                                                           | Compose browser confirmed slot-only creation, visible Recipe search, a top-toolbar generation action, and a two-click one/two-month contiguous range picker                                                   | `028814c4`, `4c0d8065`                                                                   |
| Catalogue history                | Recipe and Ingredient Vitest; ESLint and TypeScript                                                                                                                           | Compose browser confirmed Recipe and Ingredient detail URLs, Back-to-close behavior, and detail restoration after direct navigation or reload                                                                 | `0c13a373`                                                                               |
| Final verification               | 108 Vitest tests; 308 PHPUnit tests / 2,976 assertions; ESLint, Prettier, TypeScript, build, Pint, CS, and PHPStan                                                            | All implementation gates passed; temporary browser-test users and Families were removed                                                                                                                       | `2d7d6fda` test correction                                                               |
| Calendar title regression        | 8 Calendar Vitest tests; focused ESLint, Prettier, and TypeScript                                                                                                             | Compose browser confirmed a two-line clamped long Recipe name with a reserved action gutter and no **Souhrn** or **Ruční přepis** labels                                                                      | `d0bf1d17`                                                                               |
| Lazy relation foundation         | 4 Relation Search PHPUnit tests / 39 assertions; 15 Calendar PHPUnit tests / 250 assertions; 10 relation/calendar Vitest tests; CS, PHPStan, ESLint, Prettier, and TypeScript | Compose browser showed the Calendar and Ingredient selectors issuing their Family-scoped search requests only when opened                                                                                     | `a8815bcd`, `ae3c55e1`, `d0ae4240`                                                       |
| Layered Store placement creation | 36 Store/Section/Placement PHPUnit tests / 322 assertions; 23 relation/Ingredient/Store Vitest tests; CS, PHPStan, ESLint, Prettier, and TypeScript                           | Compose browser created a Store and associated Section over an unfinished Ingredient, preserved parent values, selected both fresh entities without reload, and removed the temporary Store/Section afterward | `2a8f6e84`                                                                               |
