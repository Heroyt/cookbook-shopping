...
- [Cookbook routes](../../routes/cookbook.php) expose Current-Family-scoped Store listing, creation, renaming, and deletion plus Store Section creation.
...
Store deletion reuses that dependency direction without widening its input. `DELETE stores/{store}` enters `StoreDestroyRequest`; `StoreController::destroy` passes only the authenticated User and route Store identifier to `DeleteStore`. The action resolves the Store inside `CurrentFamilyScope`, returns not found for another Family, and hard-deletes the resolved Store. The controller redirects to the Stores index with the internal `Store deleted.` translation key, rendered to the User as **Obchod byl smazán.** No Ingredient or Store–Section association persistence exists, so future placement-clearing behavior remains planned rather than simulated by this tracer.

Store Section creation follows the same boundary. `POST store-sections` enters `StoreSectionStoreRequest`, which normalizes the display name and validates a required six-digit hexadecimal colour without accepting a Family identifier. `StoreSectionController::store` passes the authenticated User, name, and colour to `CreateStoreSection`. The action derives ownership exclusively through `CurrentFamilyScope`, persists the ADR 0007 normalized key, and converts a uniqueness collision into a Czech `name` field error. The shared Stores index lists only the Current Family's Sections and renders both a colour swatch and its hexadecimal text; association, ordering, deletion, and icons remain outside this tracer.
...
The current frontend uses Vue single-file components, TypeScript, Inertia page resolution, Tailwind CSS, and shadcn-vue components. Family, Store, and Store Section management compose Inertia forms and visits with generated Wayfinder actions and installed shadcn-vue form, card, table, empty-state, dialog, and alert-dialog primitives. The Store Section form uses the native accessible colour input inside those primitives, while the list pairs the swatch with visible hexadecimal text. Generated Wayfinder modules live under ignored/generated paths in `resources/js/actions`, `resources/js/routes`, and `resources/js/wayfinder`; regenerate them rather than hand-editing them.
