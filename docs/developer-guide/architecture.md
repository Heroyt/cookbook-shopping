...
- [Cookbook routes](../../routes/cookbook.php) expose Current-Family-scoped Store listing, creation, renaming, and deletion; Store Section creation and deletion; and Store–Section attach, removal, and reorder commands.
...
Reusable Section deletion enters `StoreSectionDestroyRequest` through `DELETE store-sections/{storeSection}` and passes only the authenticated User and route identifier to `DeleteStoreSection`. Inside `CurrentFamilyScope`, the action locks the Section and its affected Stores, removes all associations, rewrites remaining positions contiguously, advances affected order versions, and deletes the Section in one transaction. The index supplies each Section's association count to a shadcn-vue AlertDialog, which also reports zero Ingredient placements because that persistence does not exist. Cross-Family identifiers are not found, client Family input is ignored, and the normalized name becomes reusable. Future Ingredient-placement clearing will extend this Cookbook action without introducing persistence into Shopping Generation.
