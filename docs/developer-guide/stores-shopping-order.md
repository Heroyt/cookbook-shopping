...
Stores, Store Sections, Store Placements, and Shopping List grouping are not implemented. Canonical terms are in [`CONTEXT.md`](../../CONTEXT.md). The planned ownership model follows [ADR 0003](../adr/0003-scope-domain-data-to-families.md), while [Shopping List generation](shopping-generation.md) defines the calculated lines that placement organizes.

## Stores and reusable Sections

> **Planned**
>
> A Store belongs to one Family and has a case-insensitively unique name plus an optional logo. A Store Section also belongs to one Family and has a case-insensitively unique name, colour, and optional icon. Sections are reusable: several Stores may associate with the same Section entity.
>
