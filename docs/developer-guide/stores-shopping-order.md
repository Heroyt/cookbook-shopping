...
Each Store may receive a private normalized logo. Each Store Section may receive a private normalized image and may independently select one icon from the server allowlist (`apple`, `carrot`, `croissant`, `milk`, `beef`, `fish`, `snowflake`, `wine`, `cookie`, `package`, `sparkles`, or `cross`). The catalogue stores only the stable icon key; the Vue layer maps it to the matching Lucide component. Unknown icon values are rejected, and clearing the selection stores null. Upload validation, WebP variants, retrieval, replacement, and deletion follow [Private entity media](recipes-ingredients.md#private-entity-media).
...
The pure `ShoppingListGrouper` groups final Shopping List Lines first by Store and then by that Store's Section traversal order. Store groups sort deterministically by application-normalized Store name with stable identity as a tie-breaker, although that order has no domain significance. A Store's unsectioned Ingredients appear after configured Sections, and Ingredients without a Store appear after all Store groups.

Ingredient names within a Section use the same application-normalized UTF-8 byte key and stable identity comparator. Tests prove deterministic Czech and accent-distinct ordering across input permutations without claiming locale-aware dictionary collation. Alternative replacement and global re-aggregation happen before grouping, so each line appears under the final Ingredient's Store Placement. Placement never changes Required Quantity or Purchase Quantity.

The associations and integrity checks are described in [Data structure](data-structure.md#placement-and-ordering-integrity). The delivery sequence is in Slice 2 of the [Implementation roadmap](implementation-roadmap.md#slice-2-stores-and-packaged-ingredients).
