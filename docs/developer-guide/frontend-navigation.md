...
The separate **Suroviny** page creates, edits, filters, archives, restores, and lists concrete Current-Family packages. Its shadcn-vue composition provides explicit metric units, piece count, description, dependent Store/Section placement, complete optional Nutrition Profile fields, and direct Alternative controls. Laravel returns canonical display quantities and validated relationship options; Vue does not reproduce persistence rules. Archived rows expose restoration instead of editing. Ingredient media, Recipe search, weekly planning, Simple Plan, and Shopping List interfaces do not exist yet.
...
>
> Add optional Ingredient media after its concrete upload policy is approved. Recipe and Shopping List interfaces must reuse the implemented active/archive, direct Alternative, placement, and canonical-kind contracts rather than reproducing them in Vue.
>
...
Laravel feature tests cover Family, Store, Section, association, and Ingredient HTTP/Inertia boundaries, including placement composite integrity and cleanup, archive filters, Alternatives, Nutrition Profile constraints, and quantity-kind removal protection. Pure unit tests cover exact metric normalization, display, and Alternative eligibility without persistence. Vitest SSR/source-contract coverage verifies generated actions and accessible Czech forms for Stores, Sections, packaged Ingredients, placement, nutrition, alternatives, and archive/restore. The full frontend suite and production build verify type-checking and bundling; pointer/keyboard interaction and live toast announcements are not yet exercised in a browser workflow test.
