...
The Stores page supports Store creation, listing, rename through a Dialog, and deletion through a consequence-stating AlertDialog. The same page supports reusable Store Section creation/listing/deletion with an accessible native colour picker, hexadecimal text alongside each swatch, and a destructive AlertDialog that discloses Store-association and Ingredient-placement counts, plus per-Store association, removal, and up/down traversal ordering. The controls expose Czech accessible names, display validation and stale-order feedback, and disable invalid boundary moves.

The separate **Suroviny** page creates and lists concrete packages in the Current Family. Its shadcn-vue Field composition provides labelled name, grams, millilitres, and piece-count inputs, Czech guidance for the mutually exclusive metric quantities, associated backend errors, processing state, and a Family-scoped Empty state. The form submits through the generated Ingredient controller action. The list renders presentation-ready canonical quantities returned by Laravel and does not reproduce conversions in Vue. Ingredient editing, input-unit selection, description, placement, media, nutrition, alternatives, archival, Recipe search, weekly planning, Simple Plan, and Shopping List interfaces do not exist yet. Those workflows described below are approved intent from the final Domain Glossary chapter, not current behavior.
...
>
> Add the remaining primary destinations for Recipes, the weekly planner, Simple Plan, and Shopping List history. Ingredients and Stores already have Current-Family-gated entries. Store Section management belongs with Store management because Sections are reusable entities whose traversal position is configured per Store. Recipe Tags belong with Cookbook maintenance rather than becoming a competing top-level taxonomy.
>
...
Laravel feature tests cover the Family, Store, Store Section, Store–Section association, and Ingredient HTTP/Inertia boundaries, validation, transaction behavior, Current Family fallback, member lifecycle, success feedback, persisted results, equal member rights, and cross-Family isolation. They prove Section entity-deletion cleanup, contiguous association positions, complete-order validation, optimistic stale-version rejection, Ingredient normalized-race conversion, quantity constraints, and fresh presentation-ready Inertia output. They call the Store and Store Section delete endpoints directly and therefore do not exercise the browser confirmations. Focused Vitest source-contract tests inspect typed Family, Store, and Ingredient actions, Store Section create/association/delete wiring, deletion composition, page composition, navigation, and account-resolution messaging. Store Section tests SSR-render the form fields, reusable list, and ordered association list to verify the labelled colour picker, Czech guidance, accessible move/remove names, provided display values, and non-colour-only output without adding a browser DOM harness. Ingredient tests SSR-render labelled canonical quantity fields, associated Czech validation errors, the empty state, and supplied list quantities; source contracts prove generated form and navigation wiring. The full frontend suite and production build verify that the composed Vue components type-check and bundle. Pointer/keyboard interaction, processing and failure state transitions, toast announcements, and Store or Store Section deletion confirmation behavior are not yet exercised in a DOM component or browser workflow test.
