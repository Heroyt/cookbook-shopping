...
Navigation uses Inertia `Link` components and generated Wayfinder route functions. Active-link behavior comes from the shared URL composable. The existing shadcn-vue primitives provide reusable sidebar, sheet, dropdown, dialog, select, tooltip, and toast behavior. Generated modules under `resources/js/actions`, `resources/js/routes`, and `resources/js/wayfinder` are build artifacts and must be regenerated rather than edited by hand.

No Family switcher, Cookbook navigation, Recipe search, weekly planner, Simple Plan, or Shopping List interface exists yet. The pages and routes described below are approved intent from the [domain glossary](../../CONTEXT.md), not current behavior.
