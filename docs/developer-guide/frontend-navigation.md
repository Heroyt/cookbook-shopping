...
The Stores page is the only Cookbook navigation and currently supports Store creation, listing, rename through a Dialog, and deletion through a consequence-stating AlertDialog. The same page supports reusable Store Section creation/listing/deletion with an accessible native colour picker, hexadecimal text alongside each swatch, and a destructive AlertDialog that discloses Store-association and Ingredient-placement counts, plus per-Store association, removal, and up/down traversal ordering. The controls expose Czech accessible names, display validation and stale-order feedback, and disable invalid boundary moves. Store Section editing, Ingredient maintenance, Recipe search, weekly planning, Simple Plan, and Shopping List interfaces do not exist yet. Those workflows described below are approved intent from the final Domain Glossary chapter, not current behavior.
...
Laravel feature tests cover the Family, Store, Store Section, and Store–Section association HTTP/Inertia boundaries, validation, transaction behavior, Current Family fallback, member lifecycle, success feedback, persisted results, equal member rights, and cross-Family isolation. They prove Section entity-deletion cleanup, contiguous association positions, complete-order validation, optimistic stale-version rejection, and fresh ordered Inertia output. They call the Store and Store Section delete endpoints directly and therefore do not exercise the browser confirmations. Focused Vitest source-contract tests inspect typed Family and Store actions, Store Section create/association/delete wiring, deletion composition, page composition, navigation, and account-resolution messaging. Store Section tests SSR-render the form fields, reusable list, and ordered association list to verify the labelled colour picker, Czech guidance, accessible move/remove names, provided display values, and non-colour-only output without adding a browser DOM harness. The full frontend suite and production build verify that the composed Vue components type-check and bundle. Pointer/keyboard interaction, processing and failure state transitions, toast announcements, and Store or Store Section deletion confirmation behavior are not yet exercised in a DOM component or browser workflow test.
