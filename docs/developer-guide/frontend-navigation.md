...
The Stores page is the only Cookbook navigation and currently supports Store creation, listing, rename through a Dialog, and deletion through a consequence-stating AlertDialog. The same page also supports reusable Store Section creation and listing with an accessible native colour picker and hexadecimal text alongside each swatch. Store–Section association/order, Store Section editing/deletion, Ingredient maintenance, Recipe search, weekly planning, Simple Plan, and Shopping List interfaces do not exist yet. Those workflows described below are approved intent from the final Domain Glossary chapter, not current behavior.
...
Laravel feature tests cover the Family, Store, and Store Section HTTP/Inertia boundaries, validation, transaction behavior, Current Family fallback, member lifecycle, success feedback, persisted results, equal member rights, and cross-Family isolation. They call the Store delete endpoint directly and therefore do not exercise the browser confirmation. Focused Vitest source-contract tests inspect typed Family and Store actions, Store Section create wiring, Store deletion composition, page composition, navigation, and account-resolution messaging. Store Section tests additionally SSR-render the form fields and list to verify the required labelled colour picker, Czech guidance, provided display values, and non-colour-only output without adding a browser DOM harness; focused PHPUnit proves the persisted normalized display values returned through Inertia. The full frontend suite and production build verify that the composed Vue components type-check and bundle. Store deletion confirmation text and cancellation, processing and failure states, focus restoration, keyboard behavior, and toast announcement are not yet exercised in a rendered component or browser workflow test.
