...
`CurrentFamilyScope` is the reusable authorization interface for Family-owned modules. It requires the membership-validated Current Family and applies it to Eloquent queries through the owned model's `family` relationship. Cookbook uses it for Store, Store Section, Ingredient, Recipe, Recipe Tag, and private media operations. Meal Planning uses it for Calendar Entries and both generation adapters; Shopping Generation uses it for explicit Saved Shopping List persistence and history. Route parameters, client-provided Family identifiers, session keys, cookies, and stale preferences do not select ownership.
...
Store, Store Section, Ingredient, Recipe, Calendar, generation, media, and saved-history feature tests use multiple Users and two Families to prove equal member access, Current-Family-only reads and writes, foreign-Family isolation, and that client input cannot redirect ownership. They additionally prove scoped normalization/race conversion, transactional cleanup and rollback, session-state separation, composite same-Family constraints, and history isolation. Slice 8 must enter through the planned Authorized Family Context without changing this web behavior. The [Security and observability](security-observability.md) chapter provides the wider control baseline.
