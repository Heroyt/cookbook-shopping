...
final class RecipeTagController extends Controller
{
    public function __construct(
        ...
        private readonly CreateRecipeTag $createTag,
        private readonly DeleteRecipeTag $deleteTag,
    ) {}
    ...
    public function store(RecipeTagStoreRequest $request): RedirectResponse
    {
        $this->currentFamilyScope->withinContext(
            ...
            fn (AuthorizedFamilyContext $context): RecipeTag => $this->createTag->handle($context, $request->tagName()),
        );
    ...
    public function destroy(RecipeMutationRequest $request, int $recipeTag): RedirectResponse
    {
        $this->currentFamilyScope->withinContext(
            $request->authenticatedUser(),
            function (AuthorizedFamilyContext $context) use ($recipeTag): void {
                $this->deleteTag->handle($context, $recipeTag);
            },
        );
